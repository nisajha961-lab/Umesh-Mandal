...
// Sound Effects Engine using the modern Web Audio API (completely offline-safe, high-performance synth sounds)
class SoundController {
  ...
  init() {
    ...
    try {
      ...
      console.warn("Web Audio API not supported in this device environment", e);
    }
  }
  ...
  playFallbackBeep(freq, duration) {
    try {
      const osc = this.ctx.createOscillator();
      const gain = this.ctx.createGain();
      osc.type = 'sine';
...
class GameApp {
  ...
  bindEvents() {
    ...
    // Game Over actions
    document.getElementById("game-btn-revive").addEventListener("click", () => {
      this.sounds.playUiClick();
      this.handleRevive();
    });
    document.getElementById("game-btn-double").addEventListener("click", () => {
      this.sounds.playUiClick();
  ...
  updateAudioToggleUI() {
    ...
    if (btn) {
      btn.innerText = this.sounds.isMuted ? "SOUND: OFF" : "SOUND: ON";
      btn.style.borderColor = this.sounds.isMuted ? "var(--neon-magenta)" : "var(--neon-cyan)";
      btn.style.color = this.sounds.isMuted ? "var(--neon-magenta)" : "var(--neon-cyan)";
      btn.style.textShadow = this.sounds.isMuted ? "0 0 5px var(--neon-magenta)" : "0 0 5px var(--neon-cyan)";
    }
  }
